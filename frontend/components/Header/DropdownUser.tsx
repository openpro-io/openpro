...
import { GET_ME } from '@/gql/gql-queries-mutations';
import { formatUserAvatarUrl, formatUserFullName } from '@/services/utils';
      ...
      >
        <span className='hidden text-right lg:block'>
          <span className='block text-sm font-medium dark:text-white'>
            {formatUserFullName(getMe?.data?.me)}
          </span>
          <span className='block text-xs'>{getMe?.data?.me?.email}</span>
        </span>

        <span className='h-12 w-12 rounded-full'>
          {!getMe?.data?.me?.avatarUrl && (
          ...
          {(getMe?.data?.me?.avatarUrl || getMe?.data?.me?.image) && (
            <img
              src={formatUserAvatarUrl(getMe?.data?.me?.avatarUrl) ?? ''}
              alt=''
