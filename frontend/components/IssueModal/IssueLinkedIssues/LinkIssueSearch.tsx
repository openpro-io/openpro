    ...
    <div className='w-full min-w-full'>
      <Combobox value={selected} onChange={setSelected}>
        <div className='relative mt-1 rounded-lg border border-primary/20'>
          <div className='relative w-full cursor-default overflow-hidden rounded-lg bg-surface-overlay text-left shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm'>
            <Combobox.Input
              className='w-full border-none bg-surface-overlay py-2 pl-3 pr-10 text-sm leading-5 text-primary focus:ring-0'
          ...
          <Transition
            ...
            afterLeave={() => setQuery('')}
          >
            <Combobox.Options className='absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-surface-overlay py-1 shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm'>
                        ...
                        {selected ? (
                          <span
                            className={`absolute inset-y-0 left-0 flex items-center pl-1 ${
                              active ? 'text-primary' : 'text-blue-500'
                            }`}
                          >
